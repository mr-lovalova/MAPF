...
class SearchClient:
    ...
    def main(args) -> None:
        ...
        class Root(TypedDict):
            solution: list[list[Action]] | None
            constraints: list[tuple[int, int, int]]
            cost: float

        def cost(solution: list[list[Action]]| None):
            if solution is None:
                return float("inf");
            cost = 0.0
        ...
        if args.cbs:
            root: Root = { "solution": [], "constraints": [], "cost": 0 }
            for agent, _ in enumerate(initial_state.agent_rows):
                root["solution"] = search(initial_state, agent)
            root["cost"] = cost(root["solution"])
            frontier = FrontierBestFirst(HeuristicGreedy(initial_state))
            while not frontier.is_empty:
                node = frontier.pop()
                if node.is_goal_state():
                    return node["solution"] # need to adapt returned object of frontier queue to contain solution, i.e Root type.
                a1, a2, v, t = FindFirstConflict(node["solution"])
                for a in [a1, a2]:
                    m = copy.deepcopy(node)
                    m["constraints"].append((a, v, t))
                    m["solution"][a] = search(initial_State[a], m.constraints)
...
if __name__ == '__main__':
    ...
    highlevel_strategy_group = parser.add_mutually_exclusive_group()
    highlevel_strategy_group.add_argument('-naive', action='store_true', dest='naive', help='Use the Naive strategy.')
    highlevel_strategy_group.add_argument('-cbs', action='store_true', dest='cbs', help='Use the CBS strategy.')
